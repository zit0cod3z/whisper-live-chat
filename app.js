...
async function sendMessage() {
  ...
  if (sourceFile) {
    if (sourceFile.size > 6 * 1024 * 1024) {
      fileStatus.textContent = "Please keep uploads below 6MB for this lightweight room server.";
      return;
    }
...
async function toggleVoiceRecording() {
  ...
  state.mediaRecorder.addEventListener("stop", () => {
    ...
    state.selectedFile = new File([blob], `voice-note-${Date.now()}.webm`, { type: blob.type });
...
function setSelectedFile(file) {
  ...
  fileStatus.textContent = `${file.name} attached and ready to send.`;
}
